...
#include "color_balance_hls_effect.h"
#include <algorithm>
#include <cmath>

namespace FreeEffect {

static EffectRegistrar<ColorBalanceHLSEffect> s_reg("Color Balance (HLS)", "Color Correction");

ColorBalanceHLSEffect::ColorBalanceHLSEffect() {
    addParameter(EffectParameter::makeAngle("hue", "Hue", 0.0));
...
std::unique_ptr<Effect> ColorBalanceHLSEffect::clone() const {
    auto e = std::make_unique<ColorBalanceHLSEffect>();
    e->setEnabled(m_enabled);
    e->setOrder(m_order);
    return e;
}

void ColorBalanceHLSEffect::render(PixelBuffer& buffer, double time) {
    (void)time;
    ...
    for (int y = 0; y < buffer.height; y++) {
        for (int x = 0; x < buffer.width; x++) {
            ...
            p[0] = static_cast<uint8_t>(std::clamp(h2r(p2, q, h + 1.0f/3.0f) * 255.0f, 0.0f, 255.0f));
            p[1] = static_cast<uint8_t>(std::clamp(h2r(p2, q, h) * 255.0f, 0.0f, 255.0f));
            p[2] = static_cast<uint8_t>(std::clamp(h2r(p2, q, h - 1.0f/3.0f) * 255.0f, 0.0f, 255.0f));
        }
    }
}
