...
#include "drop_shadow_effect.h"
#include <algorithm>
#include <cmath>

namespace FreeEffect {

static EffectRegistrar<DropShadowEffect> s_reg("Drop Shadow", "Stylize");
...
std::vector<ParameterGroup> DropShadowEffect::getParameterGroups() const {
    return {{getName(), {
...
std::unique_ptr<Effect> DropShadowEffect::clone() const {
    auto e = std::make_unique<DropShadowEffect>();
    e->setEnabled(m_enabled);
    e->setOrder(m_order);
    return e;
}

void DropShadowEffect::render(PixelBuffer& buffer, double time) {
